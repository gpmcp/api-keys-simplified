use api_keys_simplified::{ApiKeyManagerV0, Environment, KeyStatus, SecureStringExt};
...
fn test_basic_flow() {
    ...
    let wrong_key = api_keys_simplified::SecureString::from("wrong_key".to_string().into_bytes());
    assert_eq!(
...
fn test_different_environments() {
    ...
    assert!(dev.key().as_str().contains("-dev-"));
    assert!(test.key().as_str().contains("-test-"));
    assert!(staging.key().as_str().contains("-staging-"));
    assert!(live.key().as_str().contains("-live-"));
}
