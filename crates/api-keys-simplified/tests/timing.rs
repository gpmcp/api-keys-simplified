...
fn test_checksum_constant_time() {
    ...
    let stats: Vec<TimingStats> = timings.iter()
        .map(|times| calculate_stats(times))
        .collect();
    
    // Verify all timings are similar (constant-time behavior)
    let mean_nanos: Vec<f64> = stats.iter()
        .map(|s| s.mean.as_nanos() as f64)
        .collect();
    ...
    for (i, ((_, _, desc), stat)) in test_cases.iter().zip(stats.iter()).enumerate() {
        ...
        println!("{:15} - mean: {:6.2}µs, deviation: {:5.2}%", 
                 desc, stat.mean.as_micros(), deviation);
    }
