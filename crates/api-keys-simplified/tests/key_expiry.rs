...
fn test_expiry_across_environments() {
    ...
    assert_eq!(
        manager.verify(test_expired.key(), test_expired.hash()).unwrap(),
        KeyStatus::Expired
    );
    assert_eq!(
        manager.verify(staging_valid.key(), staging_valid.hash()).unwrap(),
        KeyStatus::Valid
    );
    assert_eq!(
        manager.verify(live_expired.key(), live_expired.hash()).unwrap(),
        KeyStatus::Expired
    );
}
...
fn test_expiry_without_checksum() {
    ...
    assert_eq!(
        manager.verify(expired_key.key(), expired_key.hash()).unwrap(),
        KeyStatus::Expired
    );
    assert_eq!(
...
fn test_expiry_boundaries() {
    ...
    let key_past_1s = manager
        ...
        .unwrap();
    assert_eq!(
        manager.verify(key_past_1s.key(), key_past_1s.hash()).unwrap(),
        KeyStatus::Expired
    );
}
...
fn test_corrupted_expiry_returns_invalid() {
    ...
    if parts.len() >= 2 {
        ...
        assert!(
            result.is_ok(),
            "Corrupted expiry should not cause panic"
        );
    }
}
