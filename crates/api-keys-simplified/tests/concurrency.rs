...
fn test_concurrent_generation_and_uniqueness() {
    ...
    let unique_count = all_keys.iter().collect::<std::collections::HashSet<_>>().len();
...
fn test_concurrent_verification_and_checksum() {
    ...
    let generator = Arc::new(
        ApiKeyManager::init("pk", config, HashConfig::default()).unwrap()
    );
...
fn test_clone_safety_and_config_isolation() {
    ...
    let gen3 = ApiKeyManager::init("g3", KeyConfig::high_security(), HashConfig::high_security()).unwrap();
...
fn test_high_contention_mixed_operations() {
    ...
    let generator = Arc::new(
        ApiKeyManager::init_high_security_config("stress").unwrap()
    );
