...
fn test_checksum_prevents_expensive_verification() {
    ...
    println!("✅ Invalid checksum rejected in {}μs (Argon2 bypassed)", duration.as_micros());
}
...
fn test_valid_checksum_proceeds_to_argon2() {
    ...
    println!("✅ Valid key verified in {}ms (Argon2 executed)", duration.as_millis());
}
...
fn test_dos_protection_comparison() {
    ...
    println!("   With checksum:    {}ms for 10 invalid keys", with_checksum_time.as_millis());
    println!("   Without checksum: {}ms for 10 invalid keys", without_checksum_time.as_millis());
    println!("   Speedup: {}x faster", without_checksum_time.as_millis() / with_checksum_time.as_millis().max(1));
