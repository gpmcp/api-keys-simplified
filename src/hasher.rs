...
impl KeyHasher {
    pub fn hash(key: &str, config: &HashConfig) -> Result<String> {
        // Generate salt using OS cryptographic random source
        let mut salt_bytes = [0u8; 16];
        getrandom::fill(&mut salt_bytes)
            .map_err(|e| OperationError::HashingFailed(format!("Failed to generate salt: {}", e)))?;
